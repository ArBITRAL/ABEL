...
trans(String) when is_list(String) ->
    ...
    ets:new(abelsystem,[named_table]), %% the table that store trasnlated code
    ets:insert(abelsystem,{allmodules, CNames}),
...
body([CName|Rest]) ->
    ...
    io:format("~n===Nomarlization===~n~p",[CodeForCName]),
...
%    io:format("===== Deps code ~p and ~p ~n",[DepsCode, Need]),
    ...
    io:format("~n===Simplification=== ~n~p",[Need]),

    ErlangCode = lists:map(fun({def, P, Args, {prefix, Act, Con}}) ->
				   % genrate code for prix, Con is a process call
    				   def(P,Args) ++ "    prefix(C,V,{" ++ Act ++ ",\n\t" ++ ref(Con) ++ "}\n\t).\n\n";
    			      ({def,P, Args, {choice,_,_} = Code}) ->
    				   def(P,Args) ++ "    choice(C,V,[\n\t" ++ string:join(["{" ++ Act ++ ",\n\t" ++ ref(Con) ++ "}"  || {prefix,Act,Con} <- flatten(Code)],",\n\t") ++ "\n    ]).\n\n";
    			      ({def,P, Args, {par,_,_} = Code}) ->
				   %io:format("Def par for ~p ~n",[Code]),
    				   def(P,Args) ++ "    parallel(C,V,[\n\t" ++ string:join([ref_par(X,Args) || X <- flatten(Code)],",\n\t") ++ "\n    ]).\n\n";
    			      ({def,P, Args, {call,_,_} = Code}) ->
    				   def(P,Args) ++ "    call(C,V," ++ ref(Code,Args) ++ ").\n\n";
    			      (Other) ->
				   io:format("Other does not match ~p ~n",[Other]),
    				   Other
    			   end,
    			   Need),
    io:format("~n===Generation=== ~n~p",[ErlangCode]),
    ets:insert(CName,{body,ErlangCode}),
    body(Rest);
body([]) ->
    %% Print = "\n %% COMPONENT BEHAVIOUR " ++ atom_to_list(CName) ++ "\n",
    %% output_abel(CName,Print),
    AllNames = ets:lookup_element(abelsystem,allmodules,2),
    [file:write_file(lower_case(CName) ++ ".erl", [ets:lookup_element(CName,header,2), ets:lookup_element(CName,body,2)]) || CName <- AllNames],
    create_main().
...
eval({prefix, Left, {call, _, _} = Right}, CName, ActionState) ->
...
build_act({{output, Exps, Pred}, Upd}, CName, #state{aware = Aware}) ->
    MyAtts = my_attrs(CName),
    OtherAtts = other_attrs(CName),
    G = utils:build_apred(MyAtts,Aware),
    M = utils:build_msg(Exps,MyAtts),
    U = utils:build_update(MyAtts,Upd,[]),
    SP = utils:build_spred(OtherAtts,Pred),
    _Act = "{" ++ string:join([G,M,SP,U],",\n\t") ++ "}"; % empty set of vars
build_act({{input, Pred, Vars}, Upd}, CName, #state{aware = Aware}) ->
    MyAtts = my_attrs(CName),
    OtherAtts = other_attrs(CName),
...
preprocessing(CompDefs, Comp_inits) ->
    ...
    lists:map(fun({_, {comp, CName, [{attrs, AttList}, {obsrs, ObsList}, {beh, Behaviour, {_,Init}}]}}) -> % FULL OF SIDE-EFFECT
...
create_main() ->
    IExport = "-import(abel,[new_component/3,start_component/1]).\n-export([start/1]).\n",
    Header = "-module(main).\n" ++ IExport,
    StartF = "\nstart(N) -> \n    ok = abel:start(N),\n    %%Users initialiation and starting execution\n    ok.\n",
    file:write_file("main.erl",[Header,StartF]).
%% data_eval({token,T}) ->
%%     L = get(token),
%%     put(token, L#{T => T}),
%%     T;
...
ref_par_new(Name,A1,A2) when A1 == [] ->
    P = lower_case(Name),
    if A2 == [] ->  "fun(_V) -> " ++ P ++ "(C,_V) end";
       true -> "fun(_V) -> " ++ P ++ "(C,_V," ++ string:join(lists:map(fun(X) -> "_" ++ string:uppercase(X) end, A2), ",") ++ ") end"
    end;
ref_par_new(Name,A1,A2) when A1 =/= [] ->
    P = lower_case(Name),
...
other_attrs(_CName) ->
    ...
    _Others = lists:append([X || {_,X} <- maps:to_list(Attrs)]).

my_attrs(CName) ->
    Attrs = ets:lookup_element(abcsystem, attributes, 2),
    _My = lists:append([X || {O,X} <- maps:to_list(Attrs), O == CName]).
